...
func GetRandomNameU() string {
	name := fmt.Sprintf("%s_%s", left[rand.Intn(len(left))], right[rand.Intn(len(right))])
	return name
}
func GetRandomNameT() string {
	name := fmt.Sprintf("%s%s", strings.Title(left[rand.Intn(len(left))]), strings.Title(right[rand.Intn(len(right))]))
	return name
}
